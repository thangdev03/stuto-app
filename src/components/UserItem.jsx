...
import { cancelInvitation, getInvitation } from "../utils/friendsHandler.js";
            ...
            <div className="grow flex flex-col">
                <div className="relative">
                    <div>
                        <Link 
                        to={"/user/" + user?._id} 
                        className="text-lg font-semibold hover:underline">{user?.name}</Link>
                        <div className="flex items-center gap-2 mt-1 mb-1">
                            <MdLocationOn />
                            <p className="font-medium">{user?.location || "Chưa cập nhật"}</p>
                        </div>
                        <p><span className="font-medium">Chuyên ngành:</span> {user?.major?.name || "Chưa cập nhật"}</p>
                        <p><span className="font-medium">Hệ đào tạo:</span> {user?.study_program || "Chưa cập nhật"}</p>
                        <p><span className="font-medium">Môn học:</span> {user?.wish?.subject?.name || "Chưa cập nhật"}</p>
                        <p><span className="font-medium">Mong muốn:</span> {user?.wish?.description || "Chưa cập nhật"}</p>
                    </div>
                ...
                <div className="mt-4 flex justify-between items-center flex-wrap">
                    ...
                    {requestSenders.includes(user?._id) ? (
                        <div 
                        onClick={(e) => setIsOpenResponse(true)}
                        ...
                        >
                            ...
                            Phản hồi
                            <ul className={`absolute z-40 left-0 right-0 top-11 border border-gray-200 text-textColor shadow-md text-center rounded-md overflow-hidden
                            ${!isOpenResponse && "hidden"}`}>
                                <li className="px-4 py-2 bg-[#fcfcfc] hover:bg-[#e8e8e8]">Đồng ý</li>
                                <li className="px-4 py-2 bg-[#fcfcfc] hover:bg-[#e8e8e8]">Từ chối</li>
                            </ul>
                        </div>
